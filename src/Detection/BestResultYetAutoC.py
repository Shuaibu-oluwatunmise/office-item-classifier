...
*** MODIFIED for ROBUST AUTOCALIBRATION ***
- detect_objects now returns YOLO confidences.
- Added should_auto_calibrate gate to ensure we only take a reference 
...
class MultiObjectPoseEstimator:
    def __init__(self, yolo_model_path, camera_index=1):
        ...
        # 5. Optional: Give ORB more features
        self.orb = cv2.ORB_create(nfeatures=4000) # Bumped from 2000
        self.bf_matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)

        # Removed self.smoothing_alpha as it's now stored per-object
    ...
    # ------------------------------------------------------------------
    # 🟢 1. Detection Update: Return Confidences
    # ------------------------------------------------------------------
    ...
    def calibrate_reference(self, frame, obj_name, bbox):
        ...
        print("\n" + "="*60)
        print(f"TRIGGERING CALIBRATION: {obj_name.upper()}")
        print("="*60)
        ...
        if state["ref_descriptors"] is None or len(state["ref_keypoints"]) < 10:
            print("ERROR: Not enough features detected in this frame! Re-trying later.")
    ...
    def should_auto_calibrate(self, obj_name, bbox, confidence, frame_shape):
        ...
        # (Optional) per-object tweaking:
        if obj_name == "keyboard":
            # keyboard is long and flat: we can demand a bit more area
    ...
    def track_object_homography(self, frame, obj_name, bbox):
        ...
        # Lowe's ratio test (ratio kept at 0.75 for consistency across objects)
        good_matches = []
        for m_n in matches:
            if len(m_n) < 2:
                continue
        ...
        # Use per-object minimum matches
        ...
        # Check inlier ratio using per-object minimum
    ...
    def estimate_pose_pnp(self, obj_name, corners_2d):
        ...
        # Reprojection Error is still global (8.0mm), but this is usually fine.
        success, rvec, tvec, inliers = cv2.solvePnPRansac(
            state["object_points_3d"], # Use the specific object's 3D points
            image_points,
            self.camera_matrix,
            self.dist_coeffs,
    ...
    def draw_tracked_corners(self, frame, obj_name, corners):
        ...
        # Use a distinct color for the tracked outline
    ...
    def draw_3d_axes(self, frame, obj_name, rvec, tvec, length=100):
        ...
        # Draw object-specific color circle on origin
    ...
    def draw_pose_info(self, frame, obj_name, y_offset_start, rvec_final, tvec_final):
        ...
        # Get color for info text based on object
        ...
        return y_offset # Return the new offset for the next object

    # ------------------------------------------------------------------
    # Main Loop (REWRITTEN FOR GATED AUTOCALIBRATION)
    # ------------------------------------------------------------------
    def run(self):
        """Main loop with automatic, opportunistic calibration."""
        print("\n" + "="*60)
        print("YOLO DETECTION + HOMOGRAPHY + PnP TEST (MULTI-OBJECT)")
        print("STABILITY: RANSAC, Per-Object Thresholds, and Per-Object EMA Applied")
        print("*** AUTOCALIBRATION ENABLED: Gated by Confidence & Size ***")
        print("="*60)
        print("\nControls:")
        print("  'r' - Reset ALL calibrations (Triggers gated autocalibration)")
        print("  's' - Save frame")
        print("  ESC - Exit")
        print("="*60 + "\n")
        ...
        try:
            while True:
                ...
                # 🟢 3. DETECTION: Unpack confidences
                ...
                # Get list of objects detected in the current frame
                detected_objects = [
                ...
                for obj_name in detected_objects:
                    ...
                    # 🟢 4. AUTOCALIBRATION LOGIC (Gated)
                    if not state["calibrated"]:
                        conf = confidences.get(obj_name, 0.0)
                        
                        # Apply Heuristic Gate
                        if self.should_auto_calibrate(obj_name, bbox, conf, frame.shape):                
                            if not self.calibrate_reference(frame, obj_name, bbox):
                                # Calibration failed (e.g., not enough ORB features) – try again next frame
                                continue
                        else:
                            # Not a good moment (too small, moving, low conf) - wait
                            continue
                    
                    # Tracking proceeds only if calibrated (either previously or just now)
                    if state["calibrated"]: 
                        ...
                        if corners is not None:
                            ...
                            if rvec_raw is not None and tvec_raw is not None:
                                ...
                                # Use Per-Object Smoothing Alpha (EMA Filter)
                ...
                if key == 27:  # ESC
                    ...
                    for obj_name in self.targets:
                        ...
                        state["rvec_smooth"] = None  # Reset smoother
                        state["tvec_smooth"] = None  # Reset smoother
                    print("✓ All calibrations reset. Place objects in clearly in view for autocalibration.")
...
def main():
    ...
    for obj_name, state in estimator.targets.items():
        print(f"-> {obj_name.capitalize()}: {state['width_mm']}x{state['height_mm']}mm (Min Matches: {state['min_matches']}, Min Inlier: {state['min_inlier_ratio']}, EMA: {state['ema_alpha']})")
    print("="*60 + "\n")
