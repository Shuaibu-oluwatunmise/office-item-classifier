...
*** MODIFIED for ROBUST AUTOCALIBRATION + MULTI-FRAME BEST SHOT ***
- detect_objects returns YOLO confidences.
- should_auto_calibrate gate ensures we only consider "good" detections.
- For each uncalibrated object, we buffer several good ROIs and then pick
  the one with the most ORB keypoints as the reference (like your manual 'c').
...
class MultiObjectPoseEstimator:
    def __init__(self, yolo_model_path, camera_index=1):
        ...
        for obj_name, cfg in OBJECT_CONFIGS.items():
            ...
            self.targets[obj_name] = {
                ...
                # NEW: multi-frame calibration buffer
                "calib_buffer": [],
                "calib_buffer_size": 5,  # number of candidate frames before locking reference
            }
    ...
    # ------------------------------------------------------------------
    # 1. Detection: return bboxes + confidences
    # ------------------------------------------------------------------
    ...
    def calibrate_reference(self, frame, obj_name, bbox):
        ...
        NOTE: 'frame' can be the full frame or a cropped ROI.
        """
        state = self.targets[obj_name]
        print("\n" + "="*60)
        print(f"CALIBRATING REFERENCE FROM BEST CANDIDATE: {obj_name.upper()}")
        print("="*60)
        ...
        if state["ref_descriptors"] is None or len(state["ref_keypoints"]) < 10:
            print("ERROR: Not enough features detected in this candidate! Will retry with new buffer.")
    ...
    # ------------------------------------------------------------------
    # 2. Gate Heuristic for Autocalibration
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 3. Multi-frame accumulation for "best shot" calibration
    # ------------------------------------------------------------------
    def accumulate_and_maybe_calibrate(self, frame, obj_name, bbox):
        ...
        if len(state["calib_buffer"]) > state["calib_buffer_size"]:
            state["calib_buffer"].pop(0)

        # Not enough candidates yet
        if len(state["calib_buffer"]) < state["calib_buffer_size"]:
            return False

        # Pick the candidate with the most keypoints
        best_candidate = max(state["calib_buffer"], key=lambda c: c["n_kp"])
        roi_best = best_candidate["roi"]

        # Prepare fake bbox for the ROI (full ROI area)
        ...
        # Clear buffer regardless; if it failed, we'll refill
    ...
    # ------------------------------------------------------------------
    # Main Loop (Gated + Multi-frame Autocalibration)
    # ------------------------------------------------------------------
    def run(self):
        ...
        print("="*60)
        print("\nControls:")
        print("  'r' - Reset ALL calibrations (Buffers cleared, will recalibrate)")
        print("  's' - Save frame")
        print("  ESC - Exit")
        print("="*60 + "\n")
        ...
        try:
            while True:
                ...
                for obj_name in detected_objects:
                    ...
                    # Autocalibration (gated + multi-frame)
                    if not state["calibrated"]:
                        ...
                        if self.should_auto_calibrate(obj_name, bbox, conf, frame.shape):
                            if not self.accumulate_and_maybe_calibrate(frame, obj_name, bbox):
                                # Either still filling buffer, or calibration failed -> wait
                                continue
                        else:
                            # Not a good frame yet
                            continue
