...
*** RASPBERRY PI CSI CAMERA UPDATE 🟢 ***
- Replaced cv2.VideoCapture with GStreamer/libcamera src src src.
- Environment variables and Gst initialization added.
"""
...
class MultiObjectPoseEstimator:
    # 🟢 RPI CSI: camera_index ignored in favor of gst pipeline
    ...
    # ------------------------------------------------------------------
    # 🟢 5. RPI CSI: Refactored Start Camera for GStreamer
    # ------------------------------------------------------------------
    def start_camera(self):
        ...
        # Pipeline string matching your calibration script
        # MUST match the resolution used in camera_params.py (640x480)
        gst_str = (                
            "libcamerasrc ! "
    ...
    def detect_objects(self, frame):
        ...
        # ... (YOLO detection logic is identical) ...
        results = self.yolo_model(frame, verbose=False)
        result = results[0]
    ...
    def calibrate_reference(self, frame, obj_name, bbox):
        # ... (Calibration logic is identical) ...
        ...
        if len(frame.shape) == 2 or frame.shape[0] < frame_count: # Actually frame count isn't here, ROI check
             # If passed frame is already cropped ROI
             roi = frame
        else:
    ...
    def should_auto_calibrate(self, obj_name, bbox, confidence, frame_shape):
        # ... (Sanity gate logic is identical) ...
        if confidence < 0.7:
            return False
