...
class MultiObjectPoseEstimator:
    ...
    # ------------------------------------------------------------------
    # Detection: run YOLO once, get bbox per object
    # ------------------------------------------------------------------
    def detect_objects(self, frame):
        """
        Returns dict: name -> bbox [x1,y1,x2,y2] or None
        """
    ...
    def run(self):
        ...
        print("="*60)
        print("\nControls:")
        print("  'c' - Calibrate ALL visible objects")
        ...
        try:
            while True:
                ...
                # 1. Detect all configured objects with YOLO (Run once)
                ...
                # Create a list of objects that were detected in the current frame
                detected_objects = [
                    ...
                    if state["calibrated"]:
                        ...
                        if corners is not None:
                            ...
                            if rvec_raw is not None and tvec_raw is not None:
                                ...
                                # 🟢 4. Use Per-Object Smoothing Alpha (EMA Filter)
                                alpha = state.get("ema_alpha", 0.2) # Use the object's alpha, default to 0.2
                ...
                # Check if any object needs calibration 
                needs_calibration = any(
                    not self.targets[obj_name]["calibrated"] 
                    for obj_name in detected_objects
                )
                
                if needs_calibration:
                    cv2.putText(display_frame, "Press 'c' to Calibrate All", (10, display_frame.shape[0]-10),
                ...
                elif key == ord('c'):  # Calibrate all visible objects
                    print("\nAttempting to calibrate all visible objects...")
                    calibrated_count = 0
                    for obj_name in detected_objects:
                        bbox = bboxes[obj_name]
                        if self.calibrate_reference(frame, obj_name, bbox):
                            calibrated_count += 1
                    if calibrated_count > 0:
                        print(f"✓ Calibrated {calibrated_count} object(s). Ready for tracking!")
