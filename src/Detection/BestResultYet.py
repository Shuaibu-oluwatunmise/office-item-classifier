...
Uses your trained detection model + homography for pose estimation
WITH CALIBRATED CAMERA PARAMETERS and CENTERED AXES
"""
...
# ==============================================================================
# 🔴 CALIBRATION PARAMETERS (From user's 0.1972 error file)
# ==============================================================================
...
class NotebookPoseEstimator:
    def __init__(self, yolo_model_path, camera_index=1):
        ...
        # Order is: TL, TR, BR, BL, with Y-axis pointing UP in the object frame.
    ...
    def track_notebook_homography(self, frame, bbox):
        ...
        # ✅ FIX: Lowe's ratio test (Corrected using robust for loop)
        good_matches = []
        for m_n in matches:
            if len(m_n) < 2:
                continue
        ...
        # Adjust corners to full frame coordinates
    ...
    def run(self):
        ...
        print("CENTERED AXES AND CALIBRATED CAMERA")
        print("="*60)
        print("\nControls:")
                ...
                if bbox is not None:
                    ...
                    if not self.calibrated:
                        ...
                        if corners is not None:
                            ...
                            # Estimate pose
                            rvec, tvec = self.estimate_pose_pnp(corners)
                            
                            if rvec is not None and tvec is not None:
                                # Draw 3D axes
                                display_frame = self.draw_3d_axes(display_frame, rvec, tvec)
                                ...
                                cv2.putText(display_frame, "TRACKING OK", (10, display_frame.shape[0]-10),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                            else:
                                cv2.putText(display_frame, "Pose solve failed", (10, display_frame.shape[0]-10),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 165, 255), 2)
                        else:
                            cv2.putText(display_frame, "Homography failed", (10, display_frame.shape[0]-10),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                else:
