...
- 🟢 1. YOLO inference downscaled to 320, Conv+BN fused.
- 🟢 2. YOLO frame skipping: Only runs every 4 frames to save massive CPU cycles.
- 🟢 3. ORB features reduced to 1500 + aggressive threshold for faster detection.
- 🟢 4. Calibration buffer reduced to 3 for fasterBest-Shot selection.
"""
...
class MultiObjectPoseEstimator:
    def __init__(self, yolo_model_path):                
        ...
        # 🟢 PI OPTIMIZATION: Fuse Conv + BN for faster inference
        self.yolo_model.fuse()
        
        # 🟢 PI OPTIMIZATION: Use smaller image size for YOLO
        self.yolo_imgsz = 320 # Much faster than 640

        self.class_names = self.yolo_model.names
        print(f"Model loaded with classes: {list(self.class_names.values())}")

        # 🟢 PI OPTIMIZATION: YOLO frame skipping state
        self.frame_idx = 0
        self.yolo_every_n = 4 # Run YOLO once every 4 frames
        ...
        for obj_name, cfg in OBJECT_CONFIGS.items():
            ...
            self.targets[obj_name] = {
                ...
                # 🟢 PI OPTIMIZATION: Lighter calib buffer
                "calib_buffer_size": 3, 
            }
        ...
        # 🟢 PI OPTIMIZATION: Lighter ORB config (features 4000 -> 1500)                
        self.orb = cv2.ORB_create(
            nfeatures=1500, 
            fastThreshold=12 # Aggressive threshold = faster detection
        )                
    ...
    def start_camera(self):
        ...
        # RESOLUTION 🟢: Using 640x480 as calibrated. 
        # If still slow, change width=480, height=360 here.                
        gst_str = (                
            "libcamerasrc ! "
    ...
    def detect_objects(self, frame):
        # 🟢 PI OPTIMIZATION: Pass small imgsz to model
        results = self.yolo_model(
            frame, 
            verbose=False,
            imgsz=self.yolo_imgsz, # DOWNSIZED 320
            conf=0.4,
            iou=0.5
        )                
        result = results[0]
    ...
    # ... (Calibration & Gate logic are fast, remaining identical) ...
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Main Loop (Highly Optimized for RPI CPU 🚀)
    # ------------------------------------------------------------------
    def run(self):
        ...
        print("*** 🚀 PI CPU OPTIMIZATIONS ENABLED 🚀 ***")                
        print("="*60)
        print("\nControls:")
        ...
        try:
            while True:
                ...
                # 🟢 PI OPTIMIZATION: YOLO Frame Skipping
                self.frame_idx += 1                
                
                # Run YOLO every n frames
                run_yolo = (self.frame_idx % self.yolo_every_n == 0)                
                
                # Bootstrapping: Force YOLO if nothing is calibrated yet                
                if not any(state["calibrated"] for state in self.targets.values()):                
                    run_yolo = True                
                
                if run_yolo:                
                    # 🟢 Cheaper YOLO call (imgsz handled inside detect_objects)
                ...
                for obj_name in detected_objects:
                    ...
                    if state["calibrated"]: 
                        ...
                        # Homography Tracking (Cheaper ORB config applied in __init__)
                ...
                # Visual cue for when YOLO is actually running                
                yolo_cue = "(YOLO Running)" if run_yolo else "(Homography Track)"                
                cv2.putText(display_frame, yolo_cue, (display_frame.shape[1]-200, 50),                
                ...
                cv2.imshow('Pi Optimized Pose Est', display_frame)
