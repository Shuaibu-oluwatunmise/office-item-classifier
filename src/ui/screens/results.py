...
class ResultsScreen(tk.Frame):
    def __init__(self, parent, mode, output_dir: Path, on_back, on_home):
        ...
        action_row = tk.Frame(action_bar.master, bg=action_bar.master.cget("bg"))
        ...
        result_header = tk.Frame(results_card.master, bg=results_card.master.cget("bg"))
        result_header.pack(fill="x", pady=(0, SPACE["lg"]))

        heading(result_header, "Processed Images", level=2).pack(side="left")
        label(
            result_header,
            f"{len(imgs)} image{'s' if len(imgs) != 1 else ''}",
            variant="secondary",
            size=11
        ).pack(side="left", padx=(SPACE["sm"], 0))
        ...
        if not imgs:
            # Empty state
            empty_container = tk.Frame(results_card.master, bg=results_card.master.cget("bg"))
        ...
        # Scrollable grid
        self.grid_container = ScrollFrame(results_card.master, bg=results_card.master.cget("bg"))
    ...
    def _populate_grid(self, imgs):
        ...
        for p in imgs:
            ...
            # Image container
            img_container = tk.Frame(thumb_card, bg=COLORS["white"])
            img_container.pack(padx=SPACE["sm"], pady=(SPACE["sm"], 0))

            canvas = tk.Label(img_container, image=tkimg, bg=COLORS["white"], bd=0)
            canvas.pack()
            self._thumb_refs.append(tkimg)

            # Filename label
            name_frame = tk.Frame(thumb_card, bg=COLORS["light"])
